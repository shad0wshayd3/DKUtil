...
{
	__DEBUG("DKU_H: Attempt to load {} address by id {}", SKYRIM_SUPPORT_AE ? "AE" : "SE",
		SKYRIM_SUPPORT_AE ? a_ae : a_se);
	std::uintptr_t resolved = a_vr ? REL::ID(SKYRIM_SUPPORT_AE ? a_ae : a_se).address() : REL::ID(SKYRIM_SUPPORT_AE ? a_ae : a_se).address();
	__DEBUG("DKU_H: Resolved: {:X} | Base: {:X} | RVA: {:X}", REL::ID(SKYRIM_SUPPORT_AE ? a_ae : a_se).address(), REL::Module::get().base(), resolved - REL::Module::get().base());
